...
class Transporter extends MY_Controller
{
	...
	function index($type=''){
		...
		$data['title'] = 'transporter List';

		$this->load->view('admin/includes/_header');
		$this->load->view('admin/transporter/index', $data);
	...
	function add(){
		...
		if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					redirect(base_url('admin/transporter/add'),'refresh');
				}
				else{
					$data = array();
			...
			{				
        		...
        		$this->load->view('admin/transporter/add');
        		$this->load->view('admin/includes/_footer');
			}
	}
	...
	function edit($id=""){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('errors', $data['errors']);
				redirect(base_url('admin/transporter/edit/'.$eid),'refresh');
			}
			else{
				$data = array();
				$form_fields = get_form_fields($module_id);
				...
				$data = $this->security->xss_clean($data);
				$result = $this->transporter->edit_transporter($data, $field_id);
		...
		elseif($id==""){
			...
			$this->load->view('admin/includes/_header');
			$this->load->view('admin/transporter/edit', $data);
			$this->load->view('admin/includes/_footer');
		}		
	}
