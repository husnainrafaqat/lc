	...
	function add(){
		...
		if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					redirect(base_url('admin/bank/add'),'refresh');
				}
				else{
					$data = array();
					...
					$result = $this->bank->add_bank($data);
					if($result){
						// Activity Log 
	...
	function edit($id=""){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('errors', $data['errors']);
				redirect(base_url('admin/bank/edit/'.$eid),'refresh');
			}
			else{
				$data = array();
				$form_fields = get_form_fields($module_id);
				...
				$data = $this->security->xss_clean($data);
				$result = $this->bank->edit_bank($data, $field_id);
		...
		elseif($id==""){
			...
			$module_id = module_id($controller_name);
			$data['form_fields'] = get_form_fields($module_id);
			$data['bank'] = $this->bank->get_bank_by_id($id);			
			$this->load->view('admin/includes/_header');
