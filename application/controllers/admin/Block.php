	...
	function add(){
		...
		if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					redirect(base_url('admin/block/add'),'refresh');
				}
				else{
					...
					$data = array(
						...
						'slug' => $slug,
						'societyId' => $this->input->post('society'),
						'status' => $this->input->post('status'),
						'created_at' => date('Y-m-d : h:m:s'),
						'updated_at' => date('Y-m-d : h:m:s'),
					);
					$data = $this->security->xss_clean($data);
					$result = $this->block->add_block($data);
					if($result){
	...
	function edit($id=""){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('errors', $data['errors']);
				redirect(base_url('admin/block/edit/'.$id),'refresh');
			}
			else{
				$slug = preg_replace('/[^a-zA-Z0-9_.]/', '', $this->input->post('name'));
				$data = array(					
				...
				$data = $this->security->xss_clean($data);
				$result = $this->block->edit_block($data, $id);
		...
		elseif($id==""){
			...
			$data['block'] = $this->block->get_block_by_id($id);
