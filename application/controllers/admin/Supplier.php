...
class Supplier extends MY_Controller
{
	...
	function index($type=''){
		...
		$data['property_type'] = $this->supplier->get_property_type();
	...
	function list_filterdata(){
		...
		$data['info'] = $this->supplier->get_filterdata($society,$property,$agency,$property_type,$max_price);
	...
	function list_data(){
		...
		if($role_name=="Super Admin" || $role_name=="Admin"){
			...
			$data['info'] = $this->supplier->get_user_supplier($user_id);
		}
		$this->load->view('admin/supplier/list',$data);
	}
	...
	function get_supplier_data($id){
		$row = $this->supplier->get_supplier($id);
		
		$agency = $this->supplier->get_agency_by_id($row['agency']);
		...
		$block = $this->supplier->get_block_by_id($row['block']);
		if(isset($block) && !empty($block)){
			 $block_name = $block['name'];  
		}
		$society = $this->supplier->get_society_by_id($row['society']);
		if(isset($society) && !empty($society)){
			$society_name = $society['name'];  
		}
		$property = $this->supplier->get_property_by_id($row['property']);
		if(isset($property) && !empty($property)){
			$property_name = $property['name'];  
		}
		$pt = $this->supplier->get_property_type_by_id($row['property_type']);
	...
	function add(){
		...
		if($this->input->post('submit')){
			...
			{
				$data['fields'] = $this->supplier->get_fields();
	...
	function edit($id=""){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('errors', $data['errors']);
				redirect(base_url('admin/supplier/edit/'.$id),'refresh');
			}
			else{
				$admin_id = admin_id();
				$role_name = role_name();
				...
				$data = $this->security->xss_clean($data);
				$result = $this->supplier->edit_supplier($data, $id);
	...
	public function get_blocks($socityId=null){
        if($socityId !=null){
            $all_blocks = $this->supplier->get_blocks_by_id($socityId);
			print_r($all_blocks);
