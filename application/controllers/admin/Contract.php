...
class Contract extends MY_Controller
{
    function __construct(){
		...
		$this->load->model('admin/contract_model', 'contract');
	...
	function index($type=''){
		...
		$this->load->view('admin/contract/index', $data);
	...
	function add(){
		...
		if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					redirect(base_url('admin/contract/add'),'refresh');
				}
				else{
					$data = array();
					...
					$result = $this->contract->add_contract($data);
					if($result){
						// Activity Log 
			...
			{				
        		...
        		$this->load->view('admin/contract/add');
        		$this->load->view('admin/includes/_footer');
			}
	}
	...
	function edit($id=""){
		...
		if($this->input->post('submit')){
			...
			if ($this->form_validation->run() == FALSE) {
				...
				$this->session->set_flashdata('errors', $data['errors']);
				redirect(base_url('admin/contract/edit/'.$eid),'refresh');
			}
			else{
				$data = array();
				$form_fields = get_form_fields($module_id);
				...
				$data = $this->security->xss_clean($data);
				$result = $this->contract->edit_contract($data, $field_id);
		...
		elseif($id==""){
			...
			$module_id = module_id($controller_name);
			$data['form_fields'] = get_form_fields($module_id);
			$data['contract'] = $this->contract->get_contract_by_id($id);			
			$this->load->view('admin/includes/_header');
			$this->load->view('admin/contract/edit', $data);
			$this->load->view('admin/includes/_footer');
		}		
	}
