      ...
      <div class="card card-default color-palette-bo">
        ...
        <div class="card-body">   
           <!-- For Messages -->
            ...
            <?php echo form_open(base_url('admin/supplier/edit/'.$supplier['id']), 'class="form-horizontal needs-validation" novalidate="novalidate"' )?> 
            <div class="row">
                    <?php
                      ...
                      <div class="col-md-12">
                        ...
                        <select name="<?=$slug?>" class="form-control" <?=$req?> id="<?=$slug?>" >
                          ...
                          <?php
                            foreach($vals as $val){
                          ?>
                          <option <?php if($supplier[$slug]==$val['name']){echo "selected";} ?> value="<?=$val['name']?>"><?=$val['name']?></option>
                          <?php } ?>
                        </select>
                        <?php
                          }else if($ff['type']=='textarea'){
                        ?>
                          <textarea name="<?=$slug?>" class="form-control" <?=$req?> id="<?=$slug?>"><?=$supplier[$slug]?></textarea>                          
                        <?php }else{ ?>
                          <input type="<?=$ff['type']?>" name="<?=$slug?>" value="<?=$supplier[$slug]?>" class="form-control" <?=$req?> id="<?=$slug?>" placeholder="">
                        <?php }?>  
                      </div>
                    </div>
                    <?php        
